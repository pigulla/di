...
const I3B_BINARY = join(
    require.resolve('@digitally-imported/i3-blocklet'),
    '..',
    '..',
    'bin',
    'run'
...
async function run(): Promise<void> {
    ...
    async function print_playback_state(): Promise<void> {
        ...
        try {
            ...
            if (!playback_state) {
                ...
                spinner.info(
                    `Now playing: ${playback_state.now_playing.artist} - ${playback_state.now_playing.title}`
                )
            }
        } catch (error) {
            spinner.fail(`Error: ${error.message}`)
    ...
    async function print_favorites(): Promise<void> {
        ...
        try {
            ...
            if (favorites) {
                spinner.info(
                    `Your favorites are: ${favorites.map(channel => channel.name).join(', ')}`
                )
            } else {
